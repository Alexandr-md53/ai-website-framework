...
from showcases.lawyer.domain.models import Attorney, Service, ConsultationRequest, ConsultationStatus
...
class LawyerService:
    def __init__(self) -> None:
        ...
        self._transitions = {
            ConsultationStatus.PENDING: {ConsultationStatus.APPROVED, ConsultationStatus.REJECTED},
    ...
    def _find_request_by_id(self, request_id: uuid.UUID) -> Optional[ConsultationRequest]:
    ...
    def update_request_status(
        ...
        if req is None:
            raise LookupError(f"ConsultationRequest {request_id} not found")

        # RBAC
        if user.role == LawyerUserRole.ATTORNEY:
            if user.attorney_id is None or req.attorney_id != user.attorney_id:
                raise PermissionError("Attorney cannot update other attorney's request")

        # Transition check
        allowed = self._transitions.get(req.status, set())
        if new_status not in allowed:
            raise ValueError(f"Invalid status transition {req.status.value} -> {new_status.value}")
